...
// const forgeVerifyCertChain = (store: pki.CertificateAuthorityStore, certs: pki.Certificate[]): Promise<void> => new Promise((resolve, reject) => {
//   pki.verifyCertificateChain(store, certs, 
// });

function makeAttrs(commonName: string, previousAttrs?: pki.CertificateField[]): pki.CertificateField[] {
  if (previousAttrs) {
    // same set, but change the commonName
    return previousAttrs.map(attr => attr.name === 'commonName' ? { ...attr, value: commonName } : attr);
  }
  return [
    {
      name: "commonName",
...
async function getKeysFromPemFiles(privateKeyFile: string, publicKeyFile: string): Promise<{ privateKey: string, publicKey: string }> {
  return {
    ...
    publicKey: pki.publicKeyFromPem(await readFile(publicKeyFile, 'utf8'))
  };
}
...
export async function generateCertificateWithCA(
  ...
  const { privateKey: caPrivateKey, publicKey } = await getKeysFromPemFiles(caPrivateKeyPath, domainPublicKeyPath);
  ...
  cert.publicKey = publicKey;
  cert.setSubject(makeAttrs(domain, rootCa.subject.attributes));
